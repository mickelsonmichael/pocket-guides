...
2. Browse one or two existing guides in `games/` to understand the HTML structure (e.g., `games/pokemon-yellow.html` or `games/donkey-kong-gb.html`). These files are imported as raw strings by `src/pages/GuidePage.tsx` — see AGENTS.md for details on how the React guide renderer parses them.
3. Read `src/styles/site.css` to understand the shared classes available.
...
1. **Guide file**: `games/<slug>.html`
   ...
   - Consult the **style** skill (`.github/skills/style/SKILL.md`) to choose the correct accent palette for the game and confirm what may/may not be overridden
...
3. **Registry entry**: Add a `GuideMeta` entry in `src/guides/registry.ts`
   - Include thorough `searchTerms` keywords for search
...
> **Before customising:** consult the style skill (`.github/skills/style/SKILL.md`) to pick the correct accent palette for the game.
>
> The guide is a standalone HTML file under `games/<slug>.html`. It is **not** linked from the browser directly — the React app (`src/pages/GuidePage.tsx`) imports it as a raw string and renders its inner body through shared React components (header, sidebar, footer). That means the template below omits the CDN `<link>` tags and the `<script src="../assets/guide-header.js">` that used to live in old guides: those are handled globally by the React app. Keep the per-guide `<style>` block — it's extracted and re-injected automatically.
...
<html lang="en">
  ...
  <body>
    <div class="shell">
      ...
      <footer>
        <p>
          GAME NAME &copy; YEAR Developer / Publisher. Fan-made reference only. Information sourced
          from SOURCE.
        </p>
      </footer>
    </div>
...
```html
<div class="warn-box"><strong>Missable:</strong> This item is only available before event X.</div>

<div class="info-box"><strong>Tip:</strong> Useful context or optimization advice.</div>
```
...
Only add custom CSS classes for guide-specific column styling (e.g. `.dex-num`, `.tm-move`). See `.github/skills/style/SKILL.md` for the full table reference.
